...
def test_catalogue() -> None:
    ...
    check(len(CATALOGUE) == 14, "fourteen activated abilities", str(len(CATALOGUE)))
    check(all(a.id == key for key, a in CATALOGUE.items()), "ids match keys")
    check(all(a.name for a in CATALOGUE.values()), "every action is named")
    attacks = [a for a in CATALOGUE.values() if a.is_attack]
    ...
    non_attacks = [a for a in CATALOGUE.values() if not a.is_attack]
...
def test_surcharge() -> None:
    ...
    check(CATALOGUE["frenzy"].cost.resolve(u).stamina == 3,
          "Бешенство: documented 2 -> resolves to 3")
    # Удар щитом: documented 1 -> 2.
    check(CATALOGUE["shield_bash"].cost.resolve(u).stamina == 2,
          "Удар щитом: documented 1 -> resolves to 2")
    # Марш-бросок performs no attack, so 3 stays 3.
    check(CATALOGUE["forced_march"].cost.resolve(u).stamina == 3,
...
def test_availability() -> None:
    print("\n[4] availability")
    check(CATALOGUE["frenzy"].availability(actor(stamina=2)) is Refusal.NO_STAMINA,
          "Бешенство refused at stamina 2 (needs 3)")
    check(CATALOGUE["frenzy"].availability(actor(stamina=3)) is Refusal.OK,
          "Бешенство allowed at stamina 3")
    check(CATALOGUE["healing"].availability(actor(ammo=0)) is Refusal.NO_AMMO,
          "Целительство refused without ammo")
    check(CATALOGUE["healing"].availability(actor(ammo=1)) is Refusal.OK,
          "Целительство allowed with 1 ammo")
    check(CATALOGUE["turtle"].availability(actor(action_spent=True)) is Refusal.ACTION_SPENT,
          "action already spent this round")
    check(CATALOGUE["turtle"].availability(actor(stamina=0)) is Refusal.EXHAUSTED,
    ...
    check(CATALOGUE["frenzy"].availability(tireless) is Refusal.OK,
    ...
    check(CATALOGUE["carrion_eater"].availability(rat) is Refusal.OK,
...
def test_interactions() -> None:
    ...
    check(("Точный выстрел", 2.0) in CATALOGUE["sniper_shot"].scales,
          "Снайперский выстрел doubles Точный выстрел")
    check(CATALOGUE["shield_bash"].suppresses_counterattack,
          "Удар щитом suppresses the defender's counterattack")
    check("Бестелесный" in CATALOGUE["shield_bash"].excluded_targets,
          "Удар щитом does not affect incorporeal targets")
    check("Нежить" in CATALOGUE["healing"].excluded_targets,
          "Целительство excludes undead")
    check(CATALOGUE["power_shot"].damage_scale == 1.5
          and CATALOGUE["crushing_blow"].damage_scale == 1.5,
          "both 'полтора раза' actions carry a 1.5 damage scale")
    check(CATALOGUE["shield_bash"].damage_scale == 0.0,
