...
|---|---|---|
| 16 / R7 | Tactical combat uses whole-side phases. `g_current_battle_side` remains fixed across ordinary unit selection and command paths. `004E6530` toggles it only when called with a nonzero first argument from explicit side pass or automatic current-side exhaustion. Pass is side-level; unit-by-unit side alternation is rejected. | `EXP-R7-001`; `004EC4C0` selection loop `004F13C8..004F1453`, phase calls `004F2070..004F214D`; `004E6530` toggle `004E66EE..004E670F`. |
